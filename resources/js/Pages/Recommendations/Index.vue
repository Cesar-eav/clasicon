...
<template>
    <AuthenticatedLayout title="Mis Recomendaciones">
        <template #header>
            <div class="relative top-20 z-0 bg-white  p-4 shadow-md">
        ...
        <div class="p-6 mt-10 md:mt-14 bg-white  rounded-md shadow-md overflow-y-auto h-[calc(100vh-150px)]">
            <div v-if="recommendations.length > 0" class="space-y-4 md:overflow-x-auto">
                ...
                <table class="hidden md:table w-full table-auto">
                    <thead class="bg-gray-100 ">
                        <tr>
                            <th class="px-4 py-2 text-left text-sm font-semibold text-gray-600 ">Imagen</th>
                            ...
                            <th class="px-4 py-2 text-left text-sm font-semibold text-gray-600 ">Fecha Creación</th>
                    ...
                    <tbody>
                        <tr v-for="recommendation in recommendations" :key="recommendation.id"
                            ...
                            <td class="px-4 py-2 text-sm text-gray-700 ">{{ recommendation.title }}</td>
                            <td class="px-4 py-2 text-sm text-gray-700 ">
                                {{ recommendation.description.length > 150 ? recommendation.description.slice(0, 150) + '...' : recommendation.description }}
                            </td>
                            <td class="px-4 py-2 text-sm text-gray-700 ">{{ recommendation.category }}</td>
                            <td class="px-4 py-2 text-sm text-gray-700 "><p class="text-xs"> {{recommendation.created_at}}</p>
                            </td>
                            <td class="px-4 py-2 text-right text-sm">
                                <a :href="route('recommendations.edit', recommendation.id)"
                                    class="text-[#3c888d]  mr-4">Editar</a>
                ...
                <div v-for="recommendation in recommendations" :key="recommendation.id"
                    class="block md:hidden bg-white  rounded-lg shadow-md p-4">
                    <img :src="`/storage/${recommendation.image}`" alt="Recommendation Image"
                        class="w-full h-48 object-cover rounded-md mb-4">
                    
                    <h4 class="text-lg font-bold text-gray-800 ">{{ recommendation.title }}</h4>
            ...
            <div v-else>
                <p class="text-gray-600 ">No has creado ninguna recomendación todavía.</p>
            </div>
        </div>
    </AuthenticatedLayout>
</template>
