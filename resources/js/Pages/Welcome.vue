    ...
    <div class="relative flex items-center justify-center min-h-screen bg-cover bg-center text-white"
        ...
        <div class="relative z-10 flex flex-col md:flex-row w-full max-w-screen-xl mx-auto">
            ...
            <div class="flex-1 text-center md:text-left p-6">
                <h1 class="text-5xl md:text-7xl font-extrabold mb-4 tracking-tight leading-tight">Bienvenido a <span class="text-blue-500">Clasicón.com</span></h1>
                <p class="text-lg md:text-xl mb-8">Descubre, comparte y recomienda lo mejor en libros, películas, series y mucho más. Únete a una comunidad apasionada por el conocimiento y el entretenimiento.</p>

                <!-- Botones de inicio de sesión y registro -->
                <div class="flex flex-col md:flex-row items-center gap-4 mt-6 md:mt-12">
                    <Link v-if="!auth.user" href="/login"
                        class="bg-gradient-to-r from-blue-500 to-purple-600 w-56 h-12  text-white px-6 py-2 rounded-full hover:bg-blue-700 flex items-center justify-center gap-2 transition-transform transform hover:scale-105">
                    <FontAwesomeIcon :icon="faSignInAlt" />
                    Ingresar
                    </Link>

                    <Link v-if="!auth.user" :href="route('register')"
                        class="bg-gradient-to-r from-green-500 to-teal-500 w-56 h-12 text-white px-6 py-2 rounded-full hover:bg-green-700 flex items-center justify-center gap-2 transition-transform transform hover:scale-105">
                ...
                <div v-if="auth.user" class="mt-12">
                    <Link :href="route('dashboard')"
                        class="bg-gray-800 text-white px-6 py-2 rounded-full hover:bg-gray-900 flex items-center justify-center gap-2 transition-transform transform hover:scale-105">
            ...
            <div
                class="w-full md:w-2/3 p-4 md:p-8 bg-gray-900 bg-opacity-90 rounded-lg shadow-lg h-[75vh] flex flex-col backdrop-blur-md">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-3xl font-semibold text-white">Clasicones de hoy</h2>
                ...
                <div class="space-y-4 overflow-y-auto">
                    <div v-for="(recommendation, index) in recommendations_organic" :key="recommendation.id"
                        ...
                        <img :src="`/storage/${recommendation.image}`" alt="Recommendation Image"
                            class="w-20 h-24 object-cover rounded-md mr-4 shadow-md">
                        <div class="flex-1">
                            <h3 class="text-xl font-bold text-white">{{ recommendation.title }}</h3>
